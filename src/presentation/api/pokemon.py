...
def create_pokemon(
    pokemon: PokemonCreateDTO, service: PokemonService = Depends(get_pokemon_service)
) -> PokemonResponseDTO:
...
def level_up_pokemon(
    ...
    try:
        result = service.level_up_pokemon(pokemon_id, levels)
        if result is None:
            raise HTTPException(
                status_code=HTTPStatus.NOT_FOUND,
                detail=f"Pokemon with id {pokemon_id} not found",
...
def learn_attack(
    ...
    try:
        result = service.learn_new_attack(pokemon_id, new_attack, replace_attack)
        if result is None:
            raise HTTPException(
                status_code=HTTPStatus.NOT_FOUND,
                detail=f"Pokemon with id {pokemon_id} not found",
