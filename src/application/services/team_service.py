...
class TeamService:
    ...
    def remove_pokemon_from_team(self, trainer_id: int, pokemon_id: int) -> TeamResponseDTO:
    ...
    def _validate_add_pokemon_rules(self, dto: TeamAddPokemonDTO) -> None:
        ...
        if current_size >= 6:
            raise BusinessRuleException(
                f"Team is full. Maximum 6 Pokemon allowed, current: {current_size}"
            )
        
        existing_member = self.team_repository.get_team_member(dto.trainer_id, dto.pokemon_id)
        if existing_member:
            raise BusinessRuleException(
