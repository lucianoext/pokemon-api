...
class BackpackService:
    ...
    def remove_item_from_backpack(
        self, trainer_id: int, item_id: int, dto: BackpackRemoveItemDTO
    ) -> BackpackResponseDTO:
        current_quantity = self.backpack_repository.get_item_quantity(
            trainer_id, item_id
        )
        if current_quantity == 0:
            raise BusinessRuleException(
                f"Trainer {trainer_id} doesn't have item {item_id}"
            )
        ...
        success = self.backpack_repository.remove_item(
            trainer_id, item_id, dto.quantity
        )
    ...
    def update_item_quantity(
        self, trainer_id: int, item_id: int, dto: BackpackUpdateQuantityDTO
    ) -> BackpackResponseDTO:
        current_quantity = self.backpack_repository.get_item_quantity(
            trainer_id, item_id
        )
